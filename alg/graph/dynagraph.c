...
void
insere (lista **lst, no_ptr no, int peso)
{
...
{ // monta um grafo e testa
  ...
  if ((peso_total = componente_conexo (nos + 4, 100, nos_componente, &n_nos))
      == -1)
    return 1;
  /*  if ((peso_total = componente_conexo_sem_maximo(nos+4, nos_componente,
   * &n_nos)) == -1 ) return 1;*/
...
// -----------------------------------------------------------------------------
// renomeia_friburgo : altera nome de Friburgo para Nova Friburgo, dado um
// ponteiro para Petropolis
  ...
  for (lista_ptr v = r->conexoes; v != NULL; v = v->next)
    {
      ...
      if (i == *n_nos) // neighboring node v->no not visited, insert
        peso_total
            += v->peso + componente_conexo_sem_maximo (v->no, nos, n_nos);
    }
  // peso total apenas de uma arvore que mantem os nos conexos
  // se quiser peso total com ciclos, pode simplesmente usar as conexoes no
  // grafo original. ambas solucoes serao tidas como corretas
  return peso_total;
}
  ...
  for (lista_ptr v = r->conexoes; v != NULL; v = v->next)
    {
      ...
      if (i == *n_nos) // neighboring node v->no not visited, insert
        peso_total
            += v->peso + componente_conexo (v->no, max_conexao, nos, n_nos);
    }
  return peso_total;
}
...
//    busca elemento
//    deletar elemento na mao - assumir grafo sempre nao-direcionado - ida e
//    volta funcao para deletar elemento
